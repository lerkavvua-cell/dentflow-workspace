...
function unsyncedMessages(messages: Message[]) {
  return messages.filter(message => message.syncState === 'pending' || message.syncState === 'local');
}
...
export function useDentFlowData(user: UserKey | null, onError: (message: string) => void) {
  ...
  useEffect(() => {
    ...
    if (cloudMode && db) {
      ...
      ensureFirebaseAuth().then(() => {
        ...
        const unsubMessages = onSnapshot(
          ...
          snap => {
            ...
            setMessages(prev => mergeById(cloudMessages, unsyncedMessages(prev)).sort(byCreatedAt));
          },
        ...
        const unsubPatients = onSnapshot(
          ...
          snap => {
            ...
            setPatients(cloudPatients.sort(byUpdatedAtDesc));
          },
        ...
        const unsubTasks = onSnapshot(
          ...
          snap => {
            ...
            setTasks(cloudTasks.sort(byCreatedAt));
          },
      ...
      }).catch(err => onError(`auth: ${(err as Error).message}`));
